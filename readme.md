...
Here are two ways to ship Haskell to AWS Lambda. The first method bundles shared libraries only, which is appropriate due to [quotas](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html) from AWS on lambda function package sizes. The second methods leverages the larger size quota for containers to bundle the whole set of Nix dependencies, which is more robust.
